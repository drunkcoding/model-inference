...
class TritonPythonModel:
    ...
    def initialize(self, args):
        ...
        #     : args["model_name"].rfind("_")
        # ]
        self.model_name = model_name = args["model_name"]
        model_name_or_path = os.path.join(model_repository, model_version, "model")

        self.cls_model_config = AutoConfig.from_pretrained(model_name_or_path)
        model_task = self.cls_model_config.finetuning_task
        ...
        cls_model = MODEL_TASK_TO_CLASS[model_task][model_type].from_pretrained(
            model_name_or_path
        )
    ...
    async def execute(self, requests):
        ...
        for request in requests:
            ...
            if torch.any(local_mask):
                outputs = self.model_inference(input_ids, attention_mask, local_mask) # MOVE TO CPU, SAVE GPU MEMORY
                ...
                self.logger.debug("%s local_mask updated %s", self.model_name, local_mask)
                if torch.any(local_mask):
                    batch_mask = self.update_batch_mask(
                        max_prob, batch_mask, local_mask
                    )
                    self.logger.debug("%s batch_mask updated %s", self.model_name, batch_mask)
    ...
    @torch.no_grad()
    def model_inference(self, input_ids, attention_mask, mask):
        start_time = time.perf_counter()
