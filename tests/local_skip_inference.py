...
train_len = int(len(eval_dataset) * 0.6)
...
with torch.no_grad():
    for batch in tqdm(train_dataloader, desc="Eval with Temperature"):
        ...
        for i, key in enumerate(model_keys):
            ...
            if i > 0:
                logits += model_outputs[model_keys[i-1]][-1].to(model_device[key]) * i
                logits /= i + 1
            model_outputs[key].append(logits)
            probabilities = torch.float_power(m(logits).cpu().detach(), 2)
    ...
    for batch in tqdm(test_dataloader, desc="Testing Accuracy"):
        ...
        for i, key in enumerate(model_keys):
            ...
            if hist_logits is None:
                ...
                hist_logits = (hist_logits.to(model_device[key]) * i + logits) / (i+1)
